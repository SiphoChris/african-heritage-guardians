...
import { Badge } from "@radix-ui/themes"; // Ensure Badge is imported from @radix-ui/themes
...
function AcknowledgementSection() {
  return (
    <section>
      <h3 className="text-3xl text-center font-semibold text-slate-800">
        Sponsors
      </h3>
      <div className="py-12 px-8 md:px-16 lg:px-32 bg-gray-50 border-b border-gray-200">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          ...
          {acknowledgementData.map((data) => (
            <article
              key={data.id}
              className="relative w-full h-48 rounded-md overflow-hidden card"
            >
              <Image
                src={data.image}
